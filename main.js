...
if (!message.member.permissions.has('MANAGE_ROLES')) {
  return message.reply('No tienes permiso para ejecutar este comando');
        ...
        if (!user) {
          return message.reply('Debes mencionar a un usuario para aplicarle un warn.');
        }
        ...
        db.query(`SELECT * FROM warns WHERE user_id = '${user.id}'`, (err, rows) => {
          ...
          if (warnLevel === 'leve' && warnCount.leve >= 3) {
            ...
            warnLevel = 'grave';
            applyWarn = false;
            banMessage = `⚠ | ${user.username} ha superado el máximo de warns medios. Este debería de ser aislado temporalmente y aplicarle un warn grave de acuerdo con la normativa vigente.`;
            ...
            applyWarn = false;
            banMessage = `⚠ ${user.username} debe de ser baneado por superar la cantidad máxima de warns graves. (*Todos sus datos ya han sido eliminados*)`;
          ...
          if (applyWarn) {
            ...
            db.query(`INSERT INTO warns (user_id, reason, level, staff) VALUES ('${user.id}', '${reason}', '${warnLevel}', '${message.author.id}')`, (err) => {
              if (err) {
                console.error(err);
                return message.reply('¡Ups! Ha ocurrido un error al guardar el aviso en la base de datos.');
              }
    ...
    db.query(`SELECT * FROM warns WHERE user_id = '${user_id}' ORDER BY timestamp DESC`, (err, results) => {
        if (err) {
          console.log(err);
          return message.channel.send('Error al obtener los warns.');
        }
      
        if (results.length === 0) {
            message.channel.send(`${user} no tiene warns.`);
          } else {
            let warnList = "";
            for (let i = 0; i < results.length; i++) {
              const warn = results[i];
              warnList += `${i + 1}. **Staff:** ${warn.staff} **Motivo:** ${warn.reason} **Nivel:** ${warn.level} **Fecha:** ${warn.timestamp}\n`;
            }
            message.channel.send(`Lista de warns para  ${user}:\n${warnList}`);
          }
